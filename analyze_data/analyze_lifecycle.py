...
def get_project_ids(conn):
	cur = conn.cursor()
	project_ids_sql = "select id from github_projects_commit_info where total_number_of_commits>2"
...
def get_project_commit_distribution(conn,id,date_range_buckets):
	...
	for i in range(len(date_range_buckets)-1):
		sql = "select count(*) as c from Project_commits where id ="+ \
			  str(id) +" and author_date>="+"'"+date_range_buckets[i]+"' and "+\
														   "author_date<" +"'"+date_range_buckets[i+1]+"'"
	...
	sql = "select count(*) as c from Project_commits where id =" + \
		  str(id) + " and author_date>=" + "'" + date_range_buckets[len(date_range_buckets)-1]+"'"
	logging.info("Project Commits Distribute SQL : "+ sql)
	cur.execute(sql)
	rows = cur.fetchall()
	count = int(rows[0][0])
...
def get_number_of_models_has_start_dates(models_start_end_date,bin_start_date, bin_end_date):
	ans = 0
...
def get_model_modified_commit_distribution(conn,id,date_range_buckets):
	...
	for i in range(len(date_range_buckets)-1):
		...
		rows = cur.fetchall()
		count = int(rows[0][0])
		created_models = get_number_of_models_has_start_dates(models_start_end_date,convert_str_to_date(date_range_buckets[i]),convert_str_to_date(date_range_buckets[i+1]))
		
		#modifed model commit. 
		count = count - created_models
		commit_counts.append(count)
		if total_commits==0:
	...
	rows = cur.fetchall()
	count = int(rows[0][0])
	created_models = get_number_of_models_has_start_dates(models_start_end_date,convert_str_to_date(date_range_buckets[len(date_range_buckets)-1]),datetime.now())
...
def get_model_start_and_end_dates(conn,id):
	cur = conn.cursor()
	sql = "select model_name, MIN(author_date), max(author_date) from Model_commits where id  ="+str(id)+" group by model_name"
	cur.execute(sql)
	rows = cur.fetchall()
	ans = []
	for row in rows: 
...
def convert_str_to_date(str_date):
	tmp = str_date.split(".")
	str_date_time = tmp[0]
	return datetime.strptime(str_date_time,"%Y-%m-%d %H:%M:%S")
...
def get_model_development_distribution(conn,id,date_range_buckets):
	...
	for i in range(len(date_range_buckets)-1):
		...
		for model_s_e_date in models_start_end_date:
			model_start_date = convert_str_to_date(model_s_e_date[1])
			model_end_date = convert_str_to_date(model_s_e_date[2])
			bin_start_date = convert_str_to_date(date_range_buckets[i])   
			bin_end_date = convert_str_to_date(date_range_buckets[i+1])
			if (model_start_date <= bin_start_date <= model_end_date) or (
							model_start_date <= bin_end_date <= model_end_date) or (
...
def get_model_development_distribution_deprecated(conn,id,date_range_buckets):
	cur = conn.cursor()
	total_models = get_number_of_model_under_development(conn,id)
	models_start_end_date = get_model_start_and_end_dates(conn, id)
	model_dev_ratio_distribution = []
	...
	for i in range(len(date_range_buckets)-1):
		sql = "select count(distinct(model_name)) c from Model_commits where id ="+ \
			  str(id) +"  and author_date>="+"'"+date_range_buckets[i]\
			  +"' and author_date<" +"'"+date_range_buckets[i+1]+"'"
		logging.info("Number of model under development SQL : "+ sql)
		cur.execute(sql)
		rows = cur.fetchall()
		count = int(rows[0][0])
...
def plot_all(data_lst,xlabel="Project life time", ylabel=None,figurename = None):
	...
	xtickpos = [0,4,9]
	xticksLabel = ["0-10%","40-50%","90-100%"]
	max_y_val = 70#round(max(data_lst))
	y_delta = 5
	if max_y_val>50:
		y_delta =10
	ytickpos = [ i for i in range(0,max_y_val,y_delta)]
